...
type value =
  ...
  | RecVarVal of string * string * exp * value env
...
let rec full_string_of_value v =
  ...
  | StringVal s       ->  ("\"" ^ s ^ "\"")
  | PairVal (v1,v2)   -> "("^
                         full_string_of_value v1^  ", "^
                         full_string_of_value v2^
                          ")"
  | ListVal l         -> "["^
                         (let rec pl = function
                              []     -> "]"
  ...
  | ClosureVal (x, e, m) ->
     "<\""^ x^ "\", "^
    string_of_exp e^  ", \n"^ string_of_env full_string_of_value m^
     ">\n"
  | RecVarVal (f,x, e, m)  ->
     "<<\""^ f ^ "\", \""^ x ^ "\", " ^ string_of_exp e^  ",\n"^
    string_of_env full_string_of_value m^  ">>\n"
...
let string_of_dec_result (pd, delta) = 
     "("^
    string_of_env full_string_of_value
              (List.map (function (None,x) -> ("_",x) | (Some s, x) -> (s,x)) pd)^
    ",\n"^
    string_of_env full_string_of_value delta^  ")\n"

(*
let test s = string_of_dec_result(eval_dec (parse s, []));;
*)
